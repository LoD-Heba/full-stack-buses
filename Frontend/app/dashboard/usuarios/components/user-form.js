...
import { createUser, updateUser } from "../api/api-users";
...
export function UserForm({ user }) {
  ...
  const userRoleId = user?.roles?.id || user?.roleId || "";
    ...
    async function fetchRoles() {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          console.error("Formato inesperado al obtener roles", data);
          setBackendError("Error al cargar los roles");
        }
      } catch (error) {
    ...
    <div className="max-w-lg mx-auto mt-10 p-6 border rounded-2xl shadow-md bg-white">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex justify-between items-center pt-4">
          <Button type="submit" disabled={loadingRoles}>
